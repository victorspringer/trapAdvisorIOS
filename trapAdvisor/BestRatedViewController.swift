...
class BestRatedViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.total?.text = String(format: "%.2f", Float64(touristAttractions[indexPath.row].total)!)
        
        return cell
    }
    
    private func retrieveTouristAttractions() -> [TouristAttractionRank] {
        let endPoint = "http://trapadvisor.us-east-2.elasticbeanstalk.com/v1/ta/best_rated"
        ...
        let task = session.dataTask(with: urlRequest) {
            ...
            if let httpResponse = response as? HTTPURLResponse {
                if httpResponse.statusCode == 401 {
                    self.performSegue(withIdentifier: "BestRatedToLogin", sender: nil)
                }
            }
